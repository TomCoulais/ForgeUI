<template>
    ...
    <div class="flex justify-center items-center min-h-screen w-full">
      <div class="w-full max-w-xl backdrop-blur-xl bg-slate-600/40 rounded-2xl shadow-lg border border-white/20 p-8">
        ...
        <h1 class="text-2xl font-bold text-center mb-6">Modification de votre kit</h1>
        <form @submit.prevent="submitKit">
          ...
          <div v-show="currentStep === 7" class="space-y-4">
            ...
            <div class="mt-6">
              ...
              <ButtonPreview :padding="components.buttons[0].padding" :fontSize="components.buttons[0].fontSize" :border="components.buttons[0].border" />
            </div>
              <div class="flex justify-end mt-6">
              <UButton trailing-icon="i-lucide-arrow-right" size="md" @click="nextStep() ; increment()" class="bg-fuchsia-500 hover:bg-fuchsia-700 cursor-pointer text-white">Suivant</UButton>
            </div>
          ...
          <div v-show="currentStep === 9">
            <div class="flex flex-col items-center justify-center mt-8 space-y-4">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7M16 3h-4a2 2 0 00-2 2v1h8V5a2 2 0 00-2-2z" />
                    </svg>
                    Mes kits
                </UButton>
                </div>
...
<script>
    ...
    mounted() {
        ...
        this.extractKitNameFromUrl(); 
    },

    methods: {
        extractKitNameFromUrl() {
        const kitName = this.$route.query.kitName;
        if (kitName) {
            this.fileName = kitName;
        }
        },
        ...
        async updatedKit() {
            if (!this.fileName || !this.userId) {
                alert('Nom du fichier ou utilisateur manquant.');
                return;
            }
            ...
            try {
                ...
                const response = await fetch(`http://localhost:3000/kit/updated/${this.userId}/${this.fileName}`, {
                method: 'POST',
                headers: {
                ...
                throw new Error(data.error || 'Erreur lors de la mise à jour du fichier CSS');
                }
            } catch (err) {
        ...
        hashEmail(email) {
            return crypto.subtle.digest('MD5', new TextEncoder().encode(email))
                .then(buffer => {
